...
totals <- s[,.(Sample=c("All HUD new-construction records","First address","First address, HUD coordinates by default"),
...
selection <- s[order(-confidence_loss_pct),.(State=state_name,`All new`=number(new_records),`First address`=number(first_records),
  `Former corroboration rule`=number(corroborated_records),`HUD default`=number(confident_records),`First-address reduction`=percent(first_reduction_pct),
...
reasons <- s[order(-confidence_loss_pct),.(State=state_name,`Year missing`=number(drop_year),
  `Resyndication`=number(drop_resyndication),`Scattered site`=number(drop_scattered),
  `Tied or fallback coordinates`=number(drop_coordinates),`Location unavailable`=number(drop_other_location))]
locations <- s[order(state_name),.(State=state_name,`Retained HUD points`=number(retained_hud_coordinates),
  `Census fallback`=number(retained_census_fallback),`Address unresolved, retained`=number(retained_unresolved_addresses),
  `HUD–Census distance over 500m, retained`=number(retained_coordinate_disagreements))]
...
sprintf("<p class='finding'><strong>Use HUD coordinates without requiring Census confirmation.</strong> The first-address rule reduces records by %.1f%% nationally; the subsequent checks exclude %.1f%% of first-address records. Losses range from %.1f%% in %s to %.1f%% in %s.</p>",
...
html_table(totals),
sprintf("<p>The HUD-default rule retains <strong>%s</strong> first-address records, compared with <strong>%s</strong> under the former universal-corroboration rule. Kentucky rises from %s to %s; Minnesota rises from %s to %s. HUD–Census disagreements remain diagnostic flags and do not override valid HUD coordinates.</p>",number(sum(s$confident_records)),number(sum(s$corroborated_records)),number(s[state=="KY",corroborated_records]),number(s[state=="KY",confident_records]),number(s[state=="MN",corroborated_records]),number(s[state=="MN",confident_records])),
...
"<p>The first-address rule retains the earliest known year at each standardized address. If earliest records tie, a stable HUD ID labels the address and all tied source IDs remain recorded. Different names or hedonics do not prevent selection. Unresolved addresses retain separate HUD IDs; these rows have not been established as unique addresses. Later phases at the same address are omitted by this definition; the comparison measures dependence on that assumption.</p>",
...
"<h2>Which states still lose records under the selection rules?</h2>",
"<p>A retained record needs a known 1987–2024 placed-in-service year and an available primary point. Use HUD coordinates when present and within the existing numerical bounds. When HUD coordinates are absent, an exact Census match in the reported state is the fallback. No numbered address or Census confirmation is required for a valid HUD point. Scattered-site and resyndication flags still exclude records, as do conflicting coordinates among tied earliest records. We do not exclude entire states based on their loss rates.</p>",
embedded_plot("../output/confidence_loss_pct.png","Percent of first-address records excluded under the HUD-default rules"),
...
"<h3>Coordinate sources and retained flags</h3>",html_table(locations),
"<p>Kentucky has HUD coordinates for 99 of its 102 new-construction records. The former rule excluded 87 for address text, including 85 that already had HUD coordinates. Under the HUD-default rule, Kentucky retains 94. The remaining eight exclusions are one missing year, two missing locations, four scattered-site flags, and one resyndication flag. These reasons are counted in priority order.</p>",
