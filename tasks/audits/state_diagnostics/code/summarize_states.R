...
first <- p[,.(first_records=.N,first_units=sum(total_units,na.rm=TRUE),first_units_known=sum(!is.na(total_units)),
  corroborated_records=sum(corroborated_first),
  retained_hud_coordinates=sum(confident_first & location_source=="HUD"),
  retained_census_fallback=sum(confident_first & location_source=="Census ACS2025"),
  retained_unresolved_addresses=sum(confident_first & !address_resolved),
  retained_coordinate_disagreements=sum(confident_first & location_status=="coordinate_disagreement"),
  ...
  address_missing_count=sum(!address_resolved & (is.na(street)|street=="")),
  address_multiple_count=sum(!address_resolved & grepl("MULTIPLE|SCATTERED|VARIOUS|[;&]",street)),
  unresolved_addresses=sum(!address_resolved),drop_year=sum(exclusion_reason=="missing_year"),
  ...
  drop_other_location=sum(exclusion_reason %in% c("no_location","inexact_census_match","other_tied_record_location_unavailable"))),by=.(state,year)]
