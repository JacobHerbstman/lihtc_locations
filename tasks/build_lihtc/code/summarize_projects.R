...
counts <- data.table(
  measure=c("Original new-construction records","Selected first-address records","Later or same-year records not selected","Records with unresolved first selection","Retained after confidence checks","Retained with missing total units","Retained without consistent bedroom counts","Selected missing placed-in-service year"),
  count=c(nrow(x),nrow(p),sum(x$keep_first %in% FALSE),sum(is.na(x$keep_first)),
    sum(p$confident_first),p[confident_first & is.na(total_units),.N],
