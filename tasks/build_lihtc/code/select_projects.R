...
projects <- x[keep_first %in% TRUE,.(hud_id,project_name,state_project_id,street,city,state,zip,
  ...
  location_status,location_available,address_resolved,scattered_site,resyndicated,units_conflict,bedrooms_consistent,
  records_at_address,first_record_count,selection_status,repeat_address_review,construction_review,usable_location,confident_first,corroborated_first,exclusion_reason)]
projects[,`:=`(source_hud_ids=hud_id,tied_characteristics_disagree=FALSE)]
# Consensus among the earliest tied records; a stable ID labels the address, not a preferred source.
fields <- c("project_name","state_project_id","allocation_year","total_units","low_income_units",
  "bedrooms_0","bedrooms_1","bedrooms_2","bedrooms_3","bedrooms_4plus","credit_type",
  "target_family","target_elderly","target_disabled","longitude","latitude","scattered_site","resyndicated")
...
# Keep projects; blank an inconsistent bedroom breakdown and conflicting low-income counts.
projects[, units_conflict := units_conflict | (!is.na(total_units) & !is.na(low_income_units) & low_income_units>total_units)]
projects[units_conflict==TRUE,low_income_units:=NA_real_]
bedrooms <- c("bedrooms_0","bedrooms_1","bedrooms_2","bedrooms_3","bedrooms_4plus")
projects[, bedrooms_consistent := !is.na(total_units) & rowSums(is.na(.SD))==0 &
  rowSums(.SD)==total_units & rowSums(.SD<0)==0 & rowSums(.SD!=floor(as.matrix(.SD)))==0,.SDcols=bedrooms]
